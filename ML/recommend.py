...
def getRecommendItems(userIdx, topN=5):
...
def getLeastRecommendItems(userIdx, bottomN=5):
    userScores = similarityMatrix[userIdx]
...
def getRecommend(userIdx, printAll = False):
    recommendedEvents, scores = getRecommendItems(userIdx,10)
    leastRecommendEvents, scores = getLeastRecommendItems(userIdx,5)
...
def recoverScale(df, randomness=0):
    df = df.apply(lambda x: np.array(x.strip('[]').split()).astype(np.float)) #recover embedding
...
if __name__ == '__main__':
    ...
    userkoBertEmbeddings = recoverScale(userEmbeddingsDf['koBertEmbedding'], randomness)
    itemkoBertEmbeddings = recoverScale(itemEmbeddingsDf['koBertEmbedding'], randomness)
    userkoElectraEmbeddings = recoverScale(userEmbeddingsDf['koElectraEmbedding'], randomness)
    itemkoElectraEmbeddings = recoverScale(itemEmbeddingsDf['koElectraEmbedding'], randomness)
    ...
    allRecommends = [getRecommend(userIdx, False) for userIdx in range(len(userkoBertEmbeddings))]