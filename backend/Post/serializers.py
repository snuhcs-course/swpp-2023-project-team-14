...
class Postserializer(serializers.ModelSerializer):
    ...
    event_duration = EventDurationSerializer(many=True, read_only=True, source='eventduration_set')
    club = UsernicknameSerializer(read_only=True, source='author')
    
    class Meta:
        model = Post
        fields = ('title','content','place','image','is_festival','like_count','time','event_day','club','favorite_count')